...
config = configparser.ConfigParser() # 讀config檔案
        ...
        if events[0]["type"] == "message":
            if events[0]["message"]["type"] == "text":
                ...
                if text == "我的名字":
                    ...
                    payload['messages'] = [self_introduction()]
                elif text == "台北101":
                    ...
                    payload["messages"] = [    # 文字訊息Text message
                            {
                                "type": "text",
                                "text": text
                            }
...
def getNameEmojiMessage():
    lookUpStr = string.ascii_uppercase + string.ascii_lowercase # 把字母丟入
    # lookUpStr = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
    productId = "5ac21b4f031a6752fb806d59"
    name = "Jerry"
    emojis = list()          # emojis需要是list
    text = "$" * len(name)   # 字串長度成以$
    for index, char in enumerate(name):
        emojis.append({
            "index": index,
            "productId": productId,
            "emojiId": str(lookUpStr.index(char) + 1).zfill(3)   # 會逐一回傳字母，轉為字串，並從前補0到三位數
        })
    message = dict()
    message['type'] = "text"   # 對應要求格式
    message['text'] = text
...
def getTodayCovid19Message():
    ...
    response = requests.get(url, headers=HEADER) # 抓回所有的資料
    data = response.json()[0]                    # 最新的data放在最上面，所以選第0個
    date = data["a04"]
